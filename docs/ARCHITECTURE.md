...
### 방문 집계 — 숫자 네 개가 서로 다른 것을 센다

한 칸에 섞으면 안 되므로 테이블을 나눠 뒀다.

| 값 | 어디에 쌓나 | 누가 쌓나 | 어디에 쓰나 |
| --- | --- | --- | --- |
| 그날 처음 온 **사람** | `site_visits.visitors` | 브라우저 → `/api/visit` (쿠키로 하루 1회) | 홈 통계 "오늘 방문자" |
| 열어본 횟수 | `site_visits.views` | 같음 | 관리자 화면 |
| 페이지 요청 (**사람/봇 각각**) | `site_hits` | `proxy.ts` → `record_hit()` | 관리자 화면 `/admin/visits` |
| **어디를 거쳐 왔나** | `site_referrers` | 같음 (같은 `record_hit()` 호출) | 관리자 화면 `/admin/visits` |

**봇이 브라우저 쪽 집계에 잡히지 않는 게 이유다.** 크롤러는 자바스크립트를 돌리지
않으니 `/api/visit`를 영영 부르지 않는다. 그래서 서버가 요청을 받는 자리(프록시)에서
한 번 더 센다. 판정은 `lib/visitors.ts` 한 곳에서만 한다 — 두 벌이면 홈의 방문자 수와
관리자 화면의 사람/봇 구분이 서로 어긋난다.

프록시는 **문서 요청만** 센다(`sec-fetch-dest: document`). Next는 화면 안에서 옮겨
다닐 때 RSC 요청을 따로 보내고 링크에 마우스만 올려도 미리 받아오는데, 그걸 다 세면
사람 쪽만 몇 배로 부푼다. 봇은 `sec-fetch-*`를 보내지 않으므로 헤더가 없으면 통과시킨다.
`/admin`·`/preview`는 세지 않는다.

**localhost 도 세지 않는다.** 개발 서버가 프로덕션 DB 를 가리켜서, 화면 하나 고치는
동안 "직접" 유입과 방문자가 실제 통계에 쌓였다. 사설 IP(192.168.x.x 등)도 같이 뺀다 —
폰으로 확인할 때 그 주소로 들어온다. 판정은 `isLocalHost()` 한 곳에서 하고 프록시와
`/api/visit` 가 같이 쓴다 (`lib/hits.ts`).

IP도 UA 원문도 저장하지 않는다. 남는 건 날짜·사람/봇·이름표(`googlebot`, `chrome` …)와
개수뿐이다. 개인을 되짚을 수 있는 값을 방문자 수 하나를 위해 남기는 건 값이 맞지 않는다.

**유입 경로는 사람 요청에만 붙는다.** `Referer` 헤더와 주소의 `utm_*` 를 `lib/referrers.ts`
가 갈래(`search`·`ai`·`mine`·`social`·`link`·`direct`)와 이름표(`google`·`notion`·`github` …)로
줄인다. 봇의 `Referer` 는 자기가 적어 넣은 값이라 세지 않고, 사이트 안에서 옮겨 다닌 요청도
유입이 아니라서 빠진다 — 그래서 유입 합계는 사람 요청 수보다 작다.

**경로(path)는 버린다.** 남기는 건 도메인을 줄인 이름표와, 검색엔진이 넘겨준 검색어뿐이다.
어느 글에서 왔는지까지 쌓으면 링크 하나로 사람을 좁힐 수 있게 된다.

한계를 알고 쓴다 — 구글·깃헙은 주소를 다 넘기지 않아 **검색어가 안 온다**(구글 검색어는
서치 콘솔에서 본다). 노션·카카오톡·인스타그램 앱은 `Referer` 를 아예 안 보내서 `direct`
로 들어온다. 어디에 뿌린 링크인지 꼭 알아야 하면 주소에 `?utm_source=notion` 을 붙인다 —
utm 이 헤더보다 먼저다.

**`record_hit()` 은 한 호출로 두 테이블을 쓴다.** 나누면 프록시가 요청마다 Supabase 를
두 번 부르게 된다. 인자가 늘었으므로 **마이그레이션을 먼저 올리고 코드를 배포한다** —
순서가 뒤집히면 옛 함수가 5인자 호출을 못 받아 사람/봇 집계까지 같이 멈춘다.

### 공지 이력 — 트리거로 남긴다

`notice_events`는 공지를 언제 올렸다 내렸는지를 담는다. 애플리케이션 코드가 아니라
`notices`에 붙인 트리거(`log_notice_event`)가 쓴다. 상태를 바꾸는 자리가 한 곳이 아니기
때문이다 — 수정 화면에서도 바꾸고 목록에서 바로도 바꾼다. 코드에서 남기면 경로가 늘 때마다
기록을 빠뜨린다.

본문이 어떻게 바뀌었는지는 남기지 않는다. 10개 언어 전문을 버전마다 쌓으면 이력이
...
테이블 14개 존재, 분류 5개 공개 조회, 개인 데이터가 anon에게 0행, `record_download`가
