...
| 홈 통계 | 네 칸 — 만든 것·이번 달 다운로드·최근 업데이트·**오늘 방문자**. "최근 업데이트"는 제품 발행일·릴리스·개발 기록 중 가장 나중을 쓴다. 방문자는 **오늘(KST) 값**이다 — 총합이던 것을 바꿨다. 몇 달치가 쌓인 수는 어제와 오늘이 같아 보여서 보고 나서 알게 되는 것이 없었다. 총합도 `public_stats()` 는 계속 돌려준다 |
...
| 유입 경로 | 같은 화면 아래쪽 — **검색으로 왔나, 내 노션·깃헙 링크로 왔나**를 갈래별·출처별로 가른다. 검색어는 넘겨주는 곳(네이버·다음·빙)만 쌓인다. 구글·깃헙은 주소를 다 안 보내서 출처까지만 안다. 판정은 `lib/referrers.ts` 한 곳 (`npm run test:referrers`) |
