...
| 제품 | 5개 발행. **받을 것이 있는 것 2개** — `commute-battle` v0.1.0 (Windows, 217.9MB) · `lastcall` v1.0.0-rc4 (Android, 172.9MB). **소스만 공개한 것 3개** — `gestureos-manager` · `worklog` · `je-trace` (전부 `kind='source'`, 릴리스 없음, `requires_login=false`) |
| 제품 상태 | 구현·배포를 분리해 목록/상세에 표시. 출퇴근 생존일지·살려줌 `구현 완료·배포됨`, WorkLog·GestureOS `구현 완료·미배포`, J·E TRACE `구현 완료·배포 중단`. 관리자 제품 폼에서 변경 가능 |
...
| 개발 기록 | 27건 — 출퇴근 생존일지 5건, 살려줌 5건, GestureOS 3건, WorkLog 7건, J·E TRACE 7건. 저장소 변경 이력을 사용자에게 의미 있는 기능 단위로 묶어 기록 |
...
2. **스크린샷** — 출퇴근 생존일지 6장, 살려줌 6장, GestureOS Manager 5장,
   WorkLog 7장, J·E TRACE 3장이 올라갔다. 발행 제품 5개 모두 상세 이미지가 있다.
   대체 텍스트(`alt_ko`·`alt_en`)는 출퇴근 생존일지 6장이 비어 있다 — 지금은
   제품명으로 대체된다. J·E TRACE 3장은 채웠다.
   WorkLog 는 2026-08-24 기준 실행 화면으로 교체했고 ko·en 대체 텍스트도 채웠다.
   살려줌은 저장소의 실제 테스트 캡처, GestureOS는 프런트엔드를 직접 실행해 캡처한
   대시보드·제스처 매핑·개인 학습·리듬 러쉬·휴대폰 페어링 화면이며 둘 다 ko·en
   대체 텍스트를 채웠다.
   J·E TRACE 3장은 저장소 캡처를 그대로 쓴 것이라 **역할별 미리보기 화면이고
   숫자가 전부 0 이다.** 실제 데이터가 있는 화면으로 바꾸는 게 낫다 —
   소개 본문에도 미리보기라고 적어뒀다
3. **내용 채우기** — 제품 5개, 공지 3건, 개발 기록 27건, 게시글 0건이다.
   제품별 개발 기록은 모두 한 번 이상 채웠다
4. **`je-trace` 번역 검토** — ko·en 본문을 넣었지만 `is_reviewed` 를 켜지 않았다.
