...
마지막 갱신: 2026-09-18 (살려줌 배포 중단 · 방문자 수 · 캡처 교체)
...
| DB | 테이블 15개 + `public_profiles` 뷰, RLS 전부 적용. 마이그레이션 17개 — **`20260917000001_site_visits` 는 아직 안 올렸다** (아래 "다음에 할 일") |
...
2. **`site_visits` 마이그레이션을 올린다.** 홈 방문자 칸이 여기에 걸려 있다.
   안 올리면 `/api/visit` 가 매번 404 를 로그에 남기고 숫자는 계속 0 이다
   (화면은 깨지지 않는다 — `getStats` 가 없는 열을 0 으로 읽는다).
   ```
