...
| 영역 | 상태 |
| --- | --- |
| 화면 | 홈·제품 목록·제품 상세·다운로드·공지·개발 기록·소개·로그인·약관 **전부 있음**. 좁은 화면도 메뉴가 나온다 |
...
| 관리자 | 제품·공지·개발 기록 작성, 릴리스 등록(GitHub 자동 가져오기), 게시판 목록. 메뉴 4개 전부 동작 |
...
1. **약관·개인정보처리방침 본문** — 회원을 받고 있으므로 법적으로 필요하다.
   화면이 "회원 가입을 받기 전에 올립니다" 라고 적어놓고 이미 받고 있다
2. **Storage 버킷 `product-media`** — 스크린샷 업로드. 제품 상세의 큰 자리가 계속 "준비 중" 이다
