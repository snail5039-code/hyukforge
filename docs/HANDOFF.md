...
마지막 갱신: 2026-09-18 (공지 올리기·내리기 기록 · 방문 통계 화면)
...
| DB | 테이블 17개 + `public_profiles` 뷰, RLS 전부 적용. **마이그레이션 20개 전부 원격에 올라갔다** (2026-09-18, `db-check` 통과) |
| 로그인 | **Google OAuth 동작 확인.** 관리자 1명 지정됨 |
| 관리자 | 제품·공지·개발 기록 작성, 릴리스 등록, 스크린샷 업로드, 게시판 목록, 방문 통계. 메뉴 5개 전부 동작 |
...
4. **내용 채우기** — 제품 8개, 공지 3건, 개발 기록 49건, 게시글 0건이다.
