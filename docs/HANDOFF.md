...
| 관리자 | 제품·공지·개발 기록 작성, 릴리스 등록, 스크린샷 업로드, 게시판 목록, 방문 통계. 메뉴 5개 전부 동작. **들어가는 길은 네비게이션의 `ADMIN` 버튼** — 관리자에게만 보인다(좁은 화면은 메뉴 안). 전에는 주소를 직접 쳐야만 갈 수 있었다. 버튼은 편의 장치이고 차단은 RLS 와 `/admin` 레이아웃이 한다 |
