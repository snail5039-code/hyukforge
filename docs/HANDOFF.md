...
| 제품 | 4개 발행. **받을 것이 있는 것 2개** — `commute-battle` v0.1.0 (Windows, 217.9MB) · `lastcall` v1.0.0-rc4 (Android, 172.9MB). **소스만 공개한 것 2개** — `gestureos-manager` · `worklog` (둘 다 `kind='source'`, 릴리스 없음, `requires_login=false`) |
...
2. **스크린샷** — 출퇴근 생존일지는 6장 올라갔고 홈 히어로·대표 제품 카드에
   실제 화면이 걸렸다. **살려줌 · GestureOS Manager · WorkLog 는 아직 없어서**
   상세의 큰 자리가 "준비 중" 이다.
...
3. **내용 채우기** — 제품 4개, 공지 3건, 개발 기록 10건, 게시글 0건이다.
   개발 기록은 출퇴근 생존일지·살려줌 것뿐이라 **GestureOS Manager 와 WorkLog 는
   개발 기록이 없다.** 검색·RSS·OG 는 내용이 있어야 값어치가 생긴다
4. **`worklog` 번역 검토** — ko·en 본문을 넣었지만 `is_reviewed` 를 켜지 않았다.
