...
4. **관리자가 새 글·댓글을 아는 법** — 게시판을 열어놨는데 직접 들어와야 안다.
   관리자 첫 화면에 "새 글 n건" 정도면 된다
5. **OG 이미지** — 링크를 공유하면 밋밋하게 뜬다. 제품별로 이름·버전 카드를 만들 수 있다
6. **RSS** — 개발 기록이 쌓이고 있으니 구독 경로
...
| 항목 | 왜 미뤘나 | 언제 다시 볼까 |
| --- | --- | --- |
| 업데이트 알림 메일 | 발송 수단을 붙여야 하는데 회원이 한 명이라 실익이 없다. `profiles.notify_updates` 컬럼은 남겨뒀다 | 회원이 늘고 제품이 여러 개일 때 |
| 댓글·게시글 알림 | 위와 같다. 관리자 화면의 "새 글 n건" 이 먼저다 | 게시판에 사람이 들어온 뒤 |
