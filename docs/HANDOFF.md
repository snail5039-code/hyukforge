...
2. **스크린샷** — 출퇴근 생존일지 6장, J·E TRACE 3장이 올라갔다.
   **살려줌 · GestureOS Manager · WorkLog 는 아직 없어서** 상세의 큰 자리가
   "준비 중" 이다.
   대체 텍스트(`alt_ko`·`alt_en`)는 출퇴근 생존일지 6장이 비어 있다 — 지금은
   제품명으로 대체된다. J·E TRACE 3장은 채웠다.
   WorkLog 는 저장소 README 에 걸린 이미지가 실행 화면이 아니라 발표 자료
   슬라이드라서 그대로 쓰지 않았다. 실행 화면을 새로 찍어야 한다.
   J·E TRACE 3장은 저장소 캡처를 그대로 쓴 것이라 **역할별 미리보기 화면이고
   숫자가 전부 0 이다.** 실제 데이터가 있는 화면으로 바꾸는 게 낫다 —
   소개 본문에도 미리보기라고 적어뒀다
3. **내용 채우기** — 제품 5개, 공지 3건, 개발 기록 10건, 게시글 0건이다.
   개발 기록은 출퇴근 생존일지·살려줌 것뿐이라 **GestureOS Manager · WorkLog ·
   J·E TRACE 는 개발 기록이 없다.** 검색·RSS·OG 는 내용이 있어야 값어치가 생긴다
4. **`worklog` · `je-trace` 번역 검토** — 둘 다 ko·en 본문을 넣었지만
   `is_reviewed` 를 켜지 않았다. `/ko/admin/products/<id>` 에서 읽어보고 맞으면
   언어별 "검토됨" 을 켠다
